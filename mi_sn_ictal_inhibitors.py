...
from DCEpy.Features.BurnsStudy.Mutual_Information.centralities import compute_katz, compute_eigen_centrality, compute_stats, eigen
# from DCEpy.Features.BurnsStudy.Mutual_Information.preprocessing import get_freq_bands
from DCEpy.Features.BurnsStudy.Mutual_Information.centralities import *
# from DCEpy.Features.BurnsStudy.Mutual_Information.mutual_information_noWavelet import get_seizure_time, get_seizure_windows
...
from DCEpy.Features.BurnsStudy.Permutation_MI.read_single_file import analyze_patient_raw_single_file
...
def find_centrality_multibands(training_MI_files, centrality_type = 'eig'):
...
def get_MI_features(patient_id, filename, freq_bands = ["beta"], chunk_len = 300):
    ...
    #     data_mat = scipy.io.loadmat(os.path.join("/Users/Robin/Desktop/EpilepsyVIP/DCEpy/Features/BurnsStudy/MI_features", patient_id + "CMI_5m_30shift.mat"))[filename]
    # if int(chunk_len) == 180:
    # data_mat = scipy.io.loadmat(os.path.join("/Users/Robin/Desktop/EpilepsyVIP/DCEpy/Features/BurnsStudy/MI_features", patient_id + "CMI_3m_30shift.mat"))[filename]
    data_mat = scipy.io.loadmat(os.path.join("/Users/Robin/Desktop/EpilepsyVIP/DCEpy/Features/BurnsStudy/MI_features", patient_id + "MI_1.mat"))[filename]
...
def offline_training(cv_file_type, cv_file_names, cv_file_idxs, cv_seizure_times, chunk_len, chunk_overlap,
    ...
    # for i in xrange(len(training_katz_cv_files)):
    #     pca = PCA(6, whiten=True)
    #     training_katz_cv_files[i] = pca.fit_transform(training_katz_cv_files[i])

    # print training_katz_cv_files[6][6]
    # print find_centrality_multibands(training_MI_cv_files)[6][6]
    #
    # print training_MI_cv_files[0].shape
    # print transformed_MI_cv_files[0].shape
    # print len(training_katz_cv_files)
    #
    ...
    # best_c = classifier_gridsearch_IF(training_data, cv_label_dict, seizure_indices, interictal_indices)

    # define your classifier
    best_clf = svm.OneClassSVM(nu=bestnu, kernel=svm_kernel, gamma=bestgamma)

    # best_clf = IsolationForest(contamination=best_c)

    # # train your classifier
    # pca = PCA(8,whiten=True)
    # interictal_data = pca.fit_transform(interictal_data)
...
def parent_function():
    ...
    for patient_index, patient_id in enumerate(patients):
        ...
        my_data_path = "/Users/Robin/Desktop/EpilepsyVIP/data"
        p_data_path = os.path.join(my_data_path, patient_id)


        print 'Retreiving stored raw data'
        number_of_files, data_filenames, file_type, seizure_times = fast_analyze_patient_raw(p_data_path)
        number_files = number_of_files
        ...
        for i in xrange(number_files):
            ...
            my_svm, mean_MI_matrix, sd_MI_matrix = offline_training(cv_file_type, cv_file_names,  cv_file_idxs, cv_seizure_times, chunk_len, chunk_overlap, mi_win_len, mi_win_overlap, f_s, i, patient_id,
            ...
            test_features = find_centrality_multibands(transformed_MI_test)[0]
            # print "test features b4 pca", test_features.shape
            ...
            # assert test_features.shape[1] == 20

            # plot test features for different centralities
            # plt.figure()
            # if file_type[i] == 'ictal':
            #     sz_times = get_seizure_time(patient_id, data_filenames[i])
            #     print data_filenames[i]
            #     sz_windows = get_seizure_windows(sz_times, chunk_len, chunk_overlap)
            #     plt.axvline(x=sz_windows[0],ls="dashed")
            #     plt.axvline(x =sz_windows[1], ls = "dashed")
            # for j in xrange(test_features.shape[1]):
            #     plt.plot(test_features[:,j], label = 'Channel {}'.format(j))
            # plt.legend()
            # plt.show()
            ...
            # print i
            # if file_type[i] == 'ictal':
            #     all_files, _, _, seizure_time5 = analyze_patient_raw_single_file(p_data_path, f_s, patient_id, file_type[i], i)
            #     seizure_times_samples = np.multiply(seizure_time5, f_s)
            #     plt.plot(np.multiply(test_decision_sample, 500))
            #     plt.plot(np.array(all_files)[0, :, i])
            ...
            # else:
            #     all_files, _, _, seizure_time5 = analyze_patient_raw_single_file(p_data_path, f_s, patient_id, file_type[i], i - 2)
            #
            #     plt.plot(np.multiply(test_decision_sample, 1000))
            #
            #     plt.plot(np.array(all_files)[0, :, i-2])
            # plt.show()
